...
#define StrPxDll ("px.dll")
#define StrDllFun ("start")
...
typedef void(_stdcall *lpVFunI)(int);
//typedef long double LONGDB;
...
int __stdcall WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	char ss[32]="not found ";
	HINSTANCE hDll=LoadLibrary(StrPxDll);
	if(hDll){ 
        st_=(lpVFunI)GetProcAddress(hDll,StrDllFun);		
        if(st_) { 
			st_(AppId);
			//sta=clock();
			//SetTimer(1, 1000, NULL);
        } else	{
			strcat(ss,"the entry of function in ");
			strcat(ss,StrPxDll);
			strcat(ss,".");
			MessageBox(0,ss,"error",MB_OK|MB_ICONERROR);
			exit(2);
		}
    } else	{
		strcat(ss,StrPxDll);
